# Insteon Frontend AI Agent Instructions

You are an assistant helping with development of the Home Assistant Insteon Frontend Panel. This is a TypeScript/Lit web application that provides Insteon integration management within Home Assistant.

## Insteon Stack Architecture
...
| **Insteon integration (in-tree)** | Glue code that wires Insteon into HA (`homeassistant/components/insteon`)      | https://github.com/home-assistant/core/tree/dev/homeassistant/components/insteon |
| **Insteon integration proxy**     | Issue & discussion tracker for the integration                         | https://github.com/XInsteon/insteon-integration                                      |
| **XInsteon**                      | Asynchronous Python Insteon/IP library that does the heavy lifting         | https://github.com/XInsteon/xinsteon                                                 |
| **Insteon-frontend**              | TypeScript + Lit-Element HomeAssistant panel; packed as a Python wheel | https://github.com/XInsteon/insteon-frontend                                         |

### How Components Interact

1. **Backend flow**: Integration instantiates XInsteon for Insteon/IP connection; entities proxy between HA state machine and XInsteon group-address abstractions
2. **Frontend flow**: Panel registered via `panel_custom.async_register_panel()`, communicates via WebSocket API (`/api/websocket`)
   - **Registration**: Panel loads as iframe with admin-only access, served from `/insteon_static/entrypoint.{hash}.js`
   - **Initialization**: Main component `insteon-frontend` initializes Insteon object with config entry and WebSocket services
   - **Router**: `insteon-router` handles navigation between views (info, group monitor, project, entities)
   - **WebSocket Communication**: All backend communication via `hass.callWS()` with Insteon-specific message types (`insteon/info`, `insteon/group_monitor_info`, `insteon/create_entity`, etc.)
   - **Theming**: Inherits HA themes, applies Insteon-specific CSS custom properties (`--insteon-green`, `--insteon-blue`)
   - **Components**: Built with Lit 3.x web components, uses HA design system (`<ha-*>` components)
   - **Real-time Updates**: Telegram subscription via WebSocket for live Insteon bus monitoring
3. **Release pipeline**: Wheels composition (version pins + PyPI) - no git merges required
4. **Issue tracking**: Use XInsteon/insteon-integration for integration issues, XInsteon/insteon-frontend for UI issues
...
- **Strict TypeScript**: No `any` types, define interfaces for Insteon data structures
- **Theming**: Use HA CSS variables and `this.hass.localize()` for text

### Naming Conventions

- Classes: PascalCase (`InsteonGroupMonitorPanel`)
- Variables/Functions: camelCase (`processTelegram()`)
- Private members: underscore prefix (`_buffer`)
- Elements: `insteon-` prefix for all custom elements (`insteon-group-monitor`, `insteon-telegram-info-dialog`)
...
## Insteon Domain Knowledge

### Core Concepts

- **Insteon**: Decentralized building automation protocol (EN 50090)
...
- **Telegrams**: Insteon messages sent between devices on the bus
- **DPT (Datapoint Types)**: Data formats (DPT 1.001 = boolean, DPT 9.001 = temperature)
- **ETS**: Engineering Tool Software for Insteon configuration
...
- Users import ETS project files (.insteonproj) containing group addresses and device info
- Backend (XInsteon library) handles parsing, frontend displays organized data
- Use backend APIs rather than implementing Insteon parsing in frontend
...
```typescript
@customElement("insteon-group-monitor")
class InsteonGroupMonitor extends LitElement {
...
import "../components/insteon-configure-entity";
...
- `/src`: Source code for AI agents to analyze and modify
  ...
  - `/dialogs`: Dialog components for creating/editing Insteon entities
  - `/services`: WebSocket services for backend communication
  - `/types`: TypeScript interfaces and type definitions for Insteon data structures
  - `/utils`: Utility functions for Insteon address formatting and validation
  - `/tools`: Development tools like InsteonLogger
- `/insteon_frontend`: Python package containing compiled frontend assets and entry points (AI agents should not modify directly)
...
- `make develop` or `script/develop`: Start dev server with live reload (runs gulp develop-insteon)
- Uses Gulp for bundling and hot-reload functionality

### Building

- `make build` or `script/build`: Production build (runs gulp build-insteon)
...
4. **Insteon Terminology**: Use "Group Address" not "GA", "telegram" for messages
5. **WebSocket First**: Use integration's WS commands for all backend communication
6. **Type Safety**: Define interfaces for all Insteon data structures
...
- **Code Explanation**: Use Insteon terminology and relate to HA patterns
